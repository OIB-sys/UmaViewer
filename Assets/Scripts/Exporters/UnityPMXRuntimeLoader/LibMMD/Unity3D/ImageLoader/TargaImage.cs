	...
	public class TargaImage
	{
		...
		private void LoadTGAFooterInfo(BinaryReader binReader)
		{
			...
			if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
			{
				...
				{            
					...
					string Signature = Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.FooterSignatureByteLength)).TrimEnd('\0');
					...
					if (string.Compare(Signature, TargaConstants.TargaFooterASCIISignature) == 0)
					{
						...
						string ResChar = Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.FooterReservedCharByteLength)).TrimEnd('\0');
			...
			if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
			{
				...
				if (this.objTargaFooter.ExtensionAreaOffset > 0)
				{
					try
					{
						...
						this.objTargaExtensionArea.SetAuthorName(Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.ExtensionAreaAuthorNameByteLength)).TrimEnd('\0'));
						this.objTargaExtensionArea.SetAuthorComments(Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.ExtensionAreaAuthorCommentsByteLength)).TrimEnd('\0'));
						...
						this.objTargaExtensionArea.SetJobName(Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.ExtensionAreaJobNameByteLength)).TrimEnd('\0'));
						...
						this.objTargaExtensionArea.SetSoftwareID(Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.ExtensionAreaSoftwareIDByteLength)).TrimEnd('\0'));
						
						
						// get the version number and letter from file
						float iVersionNumber = (float)binReader.ReadInt16() / 100.0F;
						string strVersionLetter = Encoding.ASCII.GetString(binReader.ReadBytes(TargaConstants.ExtensionAreaSoftwareVersionLetterByteLength)).TrimEnd('\0');
			...
			if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
			{
				if (this.objTargaHeader.ImageDataOffset > 0)
				{
					    ...
					    this.objTargaHeader.ImageType == ImageType.RUN_LENGTH_ENCODED_TRUE_COLOR)
					{
						...
						while (intImageBytesRead < intImageByteSize)
						{
							...
							if ((RLEPacketType)intRLEPacketType == RLEPacketType.RUN_LENGTH)
							{
								...
								if (bRunLengthPixel.Length == 3) { //BGR -> RGB
									var t = bRunLengthPixel [0];
									bRunLengthPixel [0] = bRunLengthPixel [2];
									bRunLengthPixel [2] = t;
								}
							...
							else if ((RLEPacketType)intRLEPacketType == RLEPacketType.RAW)
							{
								...
								for (int i = 0;i < intBytesToRead;i++)
								{
									...
									if (this.objTargaHeader.BytesPerPixel == 3 && i % 3 == 2) {
										var t = row[row.Count - 1];
										row[row.Count - 1] = row[row.Count - 3];
										row[row.Count - 3] = t;
									}
									// increment the byte counts
									intImageBytesRead++;
					...
					else
					{
						...
						{
							// Read in the row
							if (Header.BytesPerPixel == 3) { //bgr => rgb
								var rowBytes = binReader.ReadBytes(intImageRowByteSize);
								for (int j=0; j<objTargaHeader.Width;j++) {
									var pixel = j * 3;
									row.Add(rowBytes[pixel + 2]);
									row.Add(rowBytes[pixel+1]);
									row.Add(rowBytes[pixel]);
								}
							} else {
		...
		private void LoadTGAImage(BinaryReader binReader)
		{
			...
			if (pf == TextureFormat.BGRA32) {
				bmpTargaImage = new Texture2D((int)this.objTargaHeader.Width, (int)this.objTargaHeader.Height);
				bmpTargaImage.SetPixels (Utils.Bgra32ToColors(bimagedata));
			} else {
				bmpTargaImage = new Texture2D((int)this.objTargaHeader.Width, (int)this.objTargaHeader.Height, pf, false);
		...
		{
			TextureFormat targaPixelFormat = TextureFormat.ARGB32;
		...
		private void LoadThumbnail(BinaryReader binReader, TextureFormat pfPixelFormat)
		{
...
#if UNITY_EDITOR
                    GameObject.DestroyImmediate(bmpImageThumbnail);
#else
			...
			else
			{
                ...
                GameObject.DestroyImmediate(bmpImageThumbnail);
#else
        GameObject.Destroy(bmpImageThumbnail);
#endif
		...
		{
#if UNITY_EDITOR
            GameObject.DestroyImmediate(bmpTargaImage);
            GameObject.DestroyImmediate(bmpImageThumbnail);
#else
